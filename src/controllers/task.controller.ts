...
export const getTasks = async (
  req: AuthRequest<{}, { page?: string; limit?: string; status?: string; search?: string }>,
  res: Response
) => {
  try {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    if (validatedQuery.status) whereClause.status = validatedQuery.status;
...
// TOGGLE TASK STATUS
export const toggleTask = async (req: AuthRequest<{ id: string }>, res: Response) => {
  try {
